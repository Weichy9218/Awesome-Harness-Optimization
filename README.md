...
- **Harness Engineering for Self-Improvement** — Lilian Weng. *Lil'Log* 2026. [[blog]](https://lilianweng.github.io/posts/2026-07-04-harness/) — Argues the near-term loop runs through the scaffolding rather than the weights, and lists the bottlenecks that keep it from closing: fuzzy evaluators, context lifecycle, negative results, diversity collapse, reward hacking, short-horizon objectives, and where humans belong.
- **Learning Beyond Gradients** — Jiayi Weng. Blog 2026. [[post]](https://trinkle23897.github.io/learning-beyond-gradients/) — Reports programmatic policies written and iterated by a coding agent reaching Deep-RL-range scores on Atari, MuJoCo, and VizDoom with no network trained. Names the maintained object a *Heuristic System* and proposes **coupling complexity** — how many interdependent states, rules, tests, signals, and historical constraints one update must account for — explicitly not measured by lines of code. The clearest existing candidate for the behavioral-reach quantity that edit budgets currently proxy with diff size.
...
Each of these is stated as something an experiment could settle, with what would count as an answer. Where a related discussion exists we point at it, because several of these are open in the literature rather than only in this list.

**1 · What quantity actually bounds behavioral reach?** Bounded editing is the one design choice with a statistical payoff — a narrower edit language tightens the confirmation bound in [II.2](#ii2-multi-round-reuse-the-reachable-set-bound). But every system measures the bound with diff size or an edit count, and neither predicts how far behavior moves: one word can change everything, ten lines of comment nothing. Jiayi Weng's **coupling complexity** — how many interdependent states, rules, tests, signals, and prior constraints one update must account for — is the sharpest candidate we know of, and it comes with the explicit warning that lines of code is the wrong proxy. What is missing is a measurement: instrument a real loop, record a proposed edit's coupling footprint and its downstream regression rate, and check whether the correlation is strong enough to make edit budget more than an engineering limit.

**2 · Compression, not just deletion.** Most self-evolving work specifies how to write and not how to remove, and the framing is usually "when should this entry be deleted?" That is half the problem. A system that only accumulates becomes unmaintainable even when every individual entry is correct, so a healthy loop needs two operations: absorb new feedback, and fold accumulated local patches into a simpler representation. Both need a retention criterion grounded in task behavior — does this entry still cover a failure the current search neighborhood can reach? — rather than in age or length. The reporting side is equally unsolved: linking an observed regression to the accepted diff that caused it, without assuming each regression has exactly one cause.

**3 · Lifecycle contracts that cover behavior, not files.** Exact rollback is a premise of the monotonicity argument in [II.3](#ii3-what-follows-for-the-acceptance-gate), and `git revert` does not supply it: processes keep running, registrations survive, caches and written memory persist. What executable contract is sufficient to guarantee that unloading a component revokes its registrations and side effects, that a dependency change re-resolves into a working configuration, and that a rejected candidate leaves the system behaviorally where it started? Evaluation should test cleanup, replay, and cross-version recovery. Related: the systems-side treatment of revertible effects in [2608.25512](https://arxiv.org/abs/2608.25512), and the verification/recovery/state-consistency/replayability axes named by [Code as Agent Harness](https://arxiv.org/abs/2605.18747).

**4 · Confirmation under reuse, drift, and expensive tasks.** [II.2](#ii2-multi-round-reuse-the-reachable-set-bound) prices validation reuse at $\sqrt{T}$ under assumptions most systems do not meet, and rotation is cheaper in slack but costs $Tm$ tasks. Neither is affordable in settings where one rollout is a container build. Two concrete questions: does a reusable-holdout mechanism ([1411.2664](https://arxiv.org/abs/1411.2664), [Science 2015](https://www.science.org/doi/10.1126/science.aaa9375)) survive the HarnessOpt query pattern, which is more structured than general adaptive analysis — mostly accept/reject on candidates — and at what accuracy cost? And where is the crossover past which target drift, which accumulates linearly, dominates selection slack, which grows as $\sqrt{T}$, so that restarting from a fresh $s_0$ beats continuing? The second is a cheap experiment that nobody appears to have run.

**5 · Evaluators nobody can write.** Concentration bounds guarantee only the metric encoded in the loss. That covers pass rates and leaves out what actually decides whether a harness edit was good: research taste, whether a result is worth pursuing, whether a repo stays maintainable for the hundreds of engineers who share it. Standard sandbox RLVR-style training captures almost none of maintainability, ownership boundaries, migration cost, backward compatibility, or future debugging burden. Two separable problems: constructing evaluators for long-horizon and collective properties at all, and — given that any loop optimizes exactly the signal it is handed — keeping the evaluator, task data, and permissions outside the editable surface. Documented failures on the second are in [§5](#5-evaluators-and-benchmarks). How far oversight can be automated rather than staffed is itself open.

**6 · Preserving failures and preventing collapse.** These are one problem seen from two sides. Published literature is biased toward successes, so models trained on it are poor at abandoning a hypothesis, reporting a negative result, or admitting a run failed — yet failed attempts are what prune the search space fastest, and a harness should make them cheap to keep as first-class artifacts: rejected-edit records, fixed-seed replays of the failure, golden traces, explicit written-down dead directions. The other side is that selection loops concentrate on whatever currently scores well and collapse into variants of one solution. This matters most where the best path initially looks worse under the current evaluator, which is exactly the open-ended research setting these loops are being aimed at. Novelty and archive mechanisms exist ([FunSearch](https://www.nature.com/articles/s41586-023-06924-6), [ShinkaEvolve](https://arxiv.org/abs/2509.19349), MAP-Elites); what is missing is a diversity measure over harness states that predicts anything.

**7 · Which cluster partition?** [II.3](#ii3-what-follows-for-the-acceptance-gate) shows that degradation confined to a task cluster of mass $p_k$ hides under the aggregate slack until it exceeds $\eta_T/p_k$, so non-regression must be reported per cluster. "Report per cluster" is unexecutable without a defensible way to partition capabilities, and none exists. Until one does, report the partition used and why, so a reader can judge whether a tail capability could have hidden inside a large cluster.

**8 · Merging independently evolved lineages.** Two branches each pass their own gate; the merged state passed neither. The reachable-set count breaks outright, because the merge lies in neither lineage's reachable class, so the merged state may need fresh regression and safety evaluation rather than inherited approval. Beyond the statistics, the branches must be reconciled on state, dependencies, and behavior — and per-round local diffs do not predict the behavior of their composition. This is the case where the single-lineage protocol has no obvious extension.

**9 · Model–harness co-design, with the ablation that makes it checkable.** The claim worth testing is not that scaffolding produces capability. Three reported observations motivate a loop: weaker models gain more from harness optimization, no single harness is best across models, and agent capability is not determined by model strength alone. The loop: traces expose a recurring failure, the harness patches it locally or the traces become training data, fresh tasks confirm the gain, and stable experience is distilled into the model. **The test is the last step — after the model improves, can the compensating scaffolding be deleted while the fresh-task gain survives?** Without that ablation, an accumulating pile of rules is not evidence of internalized capability. Long-term progress should show up as scaffolding shrinking.

**10 · Where humans attach.** Several problems above need human judgment and none of them specifies where it enters. Human review changes write authority; it is not statistical independence, and treating it as a gate confuses governance evidence with confirmation evidence — [§4](#4-validation-protocols-how-a-candidate-enters-persistent-state) keeps them apart for this reason. The design question is which decision points are worth a human at all, at what abstraction level, and how the loop should present a candidate so that review is cheap and informative rather than a rubber stamp on a large diff.

**11 · Allocating checks by what they cost to run.** Compile, type-check, and static analysis reject candidates before any rollout, which makes filter-then-evaluate the right shape for a gate — but nobody has characterized the optimal split. Which checks can run next to execution, and which need a separate evaluator with fresh tasks, repeated trials, and audit capacity? Testable with promotion rate, validation latency, rollback cost, and cross-version failure rate. The trap to avoid is inferring independence from deployment topology: running a check on separate infrastructure does not make its data fresh.
