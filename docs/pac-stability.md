...
It is the technical appendix for README §6. The README names the two statistical questions as B1 (proposal stability) and B2 (fixed-candidate confirmation); this note supplies the assumptions, bounds, and failure modes. The companion [audit table](audit-table.md) records which state-transition protocol and runtime controls a cited system actually exposes.

The distinction is deliberate: a **state-transition gate** is an operational rule $G$ that can accept, reject, or roll back a candidate. **PAC-style confirmation** is a conditional statistical interpretation of one protocol—`separated confirmation`—when the candidate is fixed before evaluation, the confirmation sample is independent of proposal and selection, the loss is bounded, and the evaluation boundary is protected. A gate based on the search set, human review, sandboxing, or retrospective rollback may be useful, but it does not become PAC-style confirmation by name.
...
## 6. What a state-transition rule should report
...
8. for pluginized runtimes, the component version, dependency, activation, deactivation, and cleanup records;
9. a fresh final test when the selection data were reused.

Items 1 and 2 are separate properties and a system may satisfy one without the other: a clean top-level split says nothing about whether any state-transition rule can reject a candidate. Item 4 counts every state that was scored on the validation set, since selection pressure comes from evaluation rather than acceptance.
