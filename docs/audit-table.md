...
The EIP architecture direction adds lifecycle fields to this audit rather than a fourth protocol. Plugin registration, isolation, activation, and cleanup affect whether rejection restores behavior; they do not make confirmation data independent.
...
| [GEPA](https://arxiv.org/abs/2507.19457) | Pareto selection can reject candidates | `search-set` | final-test isolation should be checked per reported experiment | unverified | archive-based | parent/child scores and Pareto selection are search-time evidence, not independent confirmation |
